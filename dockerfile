...
RUN wget https://download.oracle.com/java/19/archive/jdk-19.0.2_linux-x64_bin.deb -O jdk-19_linux-x64_bin.deb
...
RUN --mount=type=cache,target=/var/cache/apt \      
    apt-get install -y --quiet --no-install-recommends \
    openjdk-19-jdk ant dos2unix ca-certificates-java dotnet-sdk-7.0 && \
    apt-get -y autoremove && \
    apt-get clean autoclean && \
    apt-get clean autoclean

RUN rm packages-microsoft-prod.deb
RUN rm jdk-19_linux-x64_bin.deb
