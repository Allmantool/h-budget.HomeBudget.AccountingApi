...
RUN wget https://download.oracle.com/java/17/latest/jdk-17_linux-x64_bin.deb -O jdk-17_linux-x64_bin.deb
...
RUN --mount=type=cache,target=/var/cache/apt \      
    apt-get install -y --quiet --no-install-recommends \
    openjdk-17-jdk ant dos2unix ca-certificates-java dotnet-sdk-7.0 && \
    apt-get -y autoremove && \
    apt-get clean autoclean && \
    apt-get clean autoclean
